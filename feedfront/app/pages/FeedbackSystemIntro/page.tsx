...
export default function FeedbackIntro(): JSX.Element {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <SuspenseFeedbackIntro />
    </Suspense>
  );
}

function SuspenseFeedbackIntro(): JSX.Element {
  ...
  useEffect(() => {
    const verify = async () => {
      const myToken =
        ...
        (await getStudentFormToken(parseInt(idForm || "")).then(
          (data) => data.token
        ));
      setToken(myToken);
      if (myToken) {
    ...
    verify();
  }, [searchParams, idForm, router, setVerified, verifyToken]);
          ...
          <div className="max-w-screen-xl m-0 sm:m-10 bg-white shadow-2xl sm:rounded-lg flex justify-center flex-1 shadow-[0_0px_64px_-12px_rgba(240,249,255,1)]">
            <div className="lg:w-1/2 p-6 sm:p-12">
              <div className="mt-12 flex flex-col items-center">
                ...
                />
                <Link
                  href={`../pages/FeedbackSystem?idForm=${idForm}&token=${token}`}
                >
                  <button
