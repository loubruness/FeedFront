 ...
 */
export async function getFormWithFieldsAction(
  request: Request,
  response: Response
) {
  try {
    ...
    if (!form) {
      return response
        .status(404)
        .json({ error: `Form with ID ${id_form} not found` });
    }
 ...
 */
export async function updateFormWithFieldsAction(
  request: Request,
  response: Response
) {
  try {
    ...
    if (!id_form || !course_name) {
      return response
        .status(400)
        .json({ error: "id_form and course_name are required" });
    }
    ...
    if (db_form.status !== "draft") {
      return response
        .status(400)
        .json({ error: `Form ${id_form} is not in draft status` });
    }
...
export async function finalizeFormAction(request: Request, response: Response) {
  try {
    ...
    console.log(`Form ${id_form} finalized`);
    console.log(process.env.SEND_EMAILS_IMMEDIATELY);
    if (process.env.SEND_EMAILS_IMMEDIATELY === "true") {
    ...
    response
      .status(200)
      .json({ message: `Form ${id_form} finalized and scheduled` });
  } catch (error) {
...
async function sendFormAction(id_form: number) {
  try {
    ...
    await sendFormToStudentsByCourseFunction(
      form.course_name,
      end_date,
      id_form
    );
 ...
 */
export async function getCoursesNamesWithoutFormAction(
  request: Request,
  response: Response
) {
  try {
    ...
    response
      .status(500)
      .json({ error: "Failed to fetch courses without forms" });
  }
}
...
async function fetchUserCoursesAction(user_id: number): Promise<Course[]> {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch user courses, status: ${response.status}`
      );
    }
    const data = await response.json();
    return data.courses;
