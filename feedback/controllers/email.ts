...
function createTokenForm(
  ...
  return sign({ user_id: user_id, user_type: user_type }, secretKeyForm, {
 ...
 */
async function sendFormEmailStudent(
  student_id: number,
  ...
  const link =
    ...
    "/pages/FeedbackSystemIntro?idForm=" +
    id_form +
    "&token=" +
    token;
...
async function fetchStudentsByCourse(id_course): Promise<Class> {
  const response = await fetch(
    `${EFREI_API_URL}/student/getStudentsByCourse/${id_course}`,
    {
      method: "GET",
      headers: {
...
async function fetchInfoCourse(id_course): Promise<Course> {
  const response = await fetch(
    `${EFREI_API_URL}/course/getInfoCourse?id_course=${id_course}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
 ...
 * @returns A promise that resolves to a message indicating the result of the email sending process.
 */
async function sendFormToStudentsByCourse(
  req: Request,
  res: Response
): Promise<void> {
  ...
  if (!id_course) {
    ...
    listStudents.students.forEach(async (student) => {
      const result = await sendFormEmailStudent(
        student.id,
        student.email,
        course.name,
      ...
      if (result !== "Message sent") {
        ...
        res
          .status(500)
          .json({ message: "An error occured while sending the emails!" });
      }
    });
    if (emailsSent) {
 ...
 * @returns A promise that resolves to a message indicating the result of the email sending process.
 */
async function sendFormToStudentsByCourseFunction(
  course_name: string,
  endDate: Date,
  id_form: number
): Promise<string> {
  const response = await fetch(
    `${EFREI_API_URL}/course/getCourseId?name=${course_name}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
 ...
 * @returns
 */
const getStudentFormTokenFunction = async (
  req: Request,
  rep: Response
): Promise<void> => {
  try {
